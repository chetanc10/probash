...
{
	if [ -z "${1}" ] || [ "$1" == "-h" ]; then
		echo2 "Usage: ${FUNCNAME[0]} <fname>\n" \
		"Find path of a file/dir walking up the dir-tree\n" \
		"<fname>: File/Directory name to find it's parent\n"
		return 1
	fi

	# Don't localize origd/curd as callers may want them later
	origd=${PWD}; curd=${PWD}

	while [ ! -d ${curd}/${1} ]; do
		if [ "${curd}" == "/" ]; then
			echo2 "No file/directory named ${1} up the directory-tree"
			return 2
		fi
		curd=$(dirname ${curd})
	done
...
geri ()
{
	...
	([ $? -ne 0 ] || [ -n "$(grep "Error in operator change:$gid" $jin)" ]) && \
